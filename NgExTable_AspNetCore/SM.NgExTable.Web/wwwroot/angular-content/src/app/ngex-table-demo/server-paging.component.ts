...
import { ApiUrlForProductList, ServerPagingDataSource } from './services/app.configs';
import {
    TableMainDirective, PaginationComponent, OptionsComponent, ColumnSortingComponent, MultiSortingCommandComponent,
    NgExTableConfig, PagingParams, TableChange, PaginationType
} from 'ngex-table'; 
...
export class ServerPagingComponent implements OnInit, OnDestroy {
    ...
    getProductList() {
        ...
        if (ServerPagingDataSource == 'server') {
            let input: any = this.getProductListRequest();
            this.httpDataService.post(ApiUrlForProductList, input).subscribe({ //Real server data
                next: (data: any) => {
            ...
            //Mock server data
            this.serverMockDataService.getPagedDataList(this.searchParams, this.pagingParams).subscribe({
                next: (data: any) => {
                    pThis.processDataReturn(data);
                },
                error: (err: HttpErrorResponse) => {
