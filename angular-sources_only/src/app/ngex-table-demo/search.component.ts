...
import {
    NgExMyDatePickerDirective, IMyDatePickerOptions, IMyDateModel, IMyInputFieldChanged, IMyCalendarViewChanged,
    IMyMarkedDate, IMyDate, IMyDefaultMonth
} from 'ngex-mydatepicker';
import { PaginationType } from 'ngex-table'; 
import * as commonMethods from 'ngex-table';
import { MyDatePickConfig } from './services/app.configs';
...
export class SearchComponent implements OnInit {
    ...
    //MyDatePicker.
    myDatePickerOptions: IMyDatePickerOptions = MyDatePickConfig;
    @ViewChild('dpFrom') dpFrom!: NgExMyDatePickerDirective;
    @ViewChild('dpTo') dpTo!: NgExMyDatePickerDirective;
    ...
    searchGo(event?: Event): void {
        ...
        //Updated ngex-mydatepicker: output from singleDate object of the model.
        //No need to check isRange here.
        if (this.search.dateFrom != "" || this.search.dateTo != "") {
            let dateFrom: any = this.search.dateFrom;
            if (dateFrom == null) {
                //validate some values such as "02/30/2014" as invalid.
                this.errorMessage += "Invalid Available From date.\n";
            }
            let dateTo: any = this.search.dateTo;
            if (dateTo == null) {
