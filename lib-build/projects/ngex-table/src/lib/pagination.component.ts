...
export class PaginationComponent implements OnInit, OnDestroy, OnChanges {
    ...
    setStartItemNumber(pageNum: number = 0) {
        let currentPageNum: number = pageNum != 0 ? pageNum : this.pageNumber;
        this.startItemNumber = (currentPageNum - 1) * this.pageSize + 1;
    }
    setEndItemNumber(pagedCount: number = 0) {
        let currentLength: number = pagedCount != 0 ? pagedCount : this.pagedLength;
