...
export class TableMainDirective implements OnInit, OnDestroy {
    ...
    //Calling components.
    private columnSortComponent: Subject<any> = new Subject<any>();
    columnSortComponent$ = this.columnSortComponent.asObservable();

    private multiSortCommandComponent: Subject<any> = new Subject<any>();
    multiSortCommandComponent$ = this.multiSortCommandComponent.asObservable();

    private sortingTypeComponent: Subject<any> = new Subject<any>();
    ...
    initSortableList(sortableItem: SortableItem) {
        ...
        if (sortableItem.sequence == -1 || sortableItem.sortDirection == '') {
            if (this.pagingParams.sortList.length > 0) {
                //SW 12/22/2021 corrected. pagingParams.sortList[1] doesn't exist when using cached params for single sorting.
                //Also for browser refresh.
                ...
                //    }
                //}
                for (let idx: number = 0; idx < this.pagingParams.sortList.length; idx++) {
                    if (this.pagingParams.sortList[idx].sortBy == sortableItem.sortBy) {
                        sortableItem.sortDirection = this.pagingParams.sortList[idx].sortDirection;
                        if (this.sortingOption == 'single') {
                            sortableItem.sequence = 1;
                        }
                        else if (this.sortingOption == 'multiple') {
                            sortableItem.sequence = idx + 1;
                        }
                        break;
                    }
                }
